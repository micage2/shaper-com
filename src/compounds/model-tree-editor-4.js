...
function getChildTypes(model, tableUuid) {
...
// AddInstanceDialog - edit-mode dialog
function AddInstanceDialog(args) {
    ...
    const toolbar = DOM.create(Toolbar, {});
    const select = Selector({ options: childTypes });
    
    const confirm = DOM.create(Button, { label: '✓' });
    confirm.on('clicked', function() {
        const childType = select.getValue();
        if (childType) {
            const table = model.getTable(childType);
            const row = table.addRow({});
            toolbar.emit('close', { tableUuid: childType, rowId: row.id });
        } else {
            toolbar.emit('close');
        }
    });
...
// DeleteInstanceDialog - edit-mode dialog
